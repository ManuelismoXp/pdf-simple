...
export const spacedTable = (pdf: jsPDF, x: number, y: number, headers: Header[], headerheight: number, dataset: DataSet[][], border: boolean = false): void => {
    ...
    headers?.forEach(head => {
        ...
        align(pdf, newX + 1, y, head.text, head.width, head.align ?? undefined);
    ...
    dataset?.forEach((data, index) => {
        ...
        data?.forEach((element, i) => {
            ...
            align(pdf, newX + 1, y, element, widths[i].width, widths[i].align ?? undefined);
